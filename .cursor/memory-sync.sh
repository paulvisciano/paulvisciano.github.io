...
if (toProcess.length === 0) {
  ...
  if (!dryRun) {
    ...
    const fingerprintJsonPath = path.join(memoryDir, 'fingerprint.json');
    ...
    fs.writeFileSync(fingerprintJsonPath, fingerprintJson);
    console.log('✅ Refreshed memory/fingerprint.json');
  }
