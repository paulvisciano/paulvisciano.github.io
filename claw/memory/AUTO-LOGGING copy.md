...
2. **Check for existing transcript.md:**
   - If `transcript.md` already exists for today, **APPEND** to it. Never overwrite.
...
## Errors from Feb 24 Session (CRITICAL FIXES)

**Error #1: Claimed execution without doing it**
- Read AUTO-LOGGING.md, understood the spec, then said "auto-logging enabled" without actually creating folders or archiving files
- **Fix:** Don't say it's done. Prove it by executing the steps. Show the evidence (archived file, transcript line)

**Error #2: Overwrote existing transcript instead of appending**
- Created fresh transcript.md three times, erasing earlier conversation
- **Fix:** ALWAYS check if transcript.md exists. If yes, append. If no, create. Use file append mode, never overwrite mode.

**Error #3: Said files were archived but didn't move them**
- Claimed "audio archived" without actually copying media from inbound cache to `/memory/raw/YYYY-MM-DD/audio/`
- **Fix:** Before every response, scan `/Users/paulvisciano/.openclaw/media/inbound/` for files newer than last processed, copy them with HHMMSS timestamps, then respond. Show the filepath in the transcript.

**Error #4: Didn't format responses correctly**
- Said things like "auto-logging live" without showing proof in the WhatsApp transcript format
- **Fix:** Every response must follow this exact format:
  ```
  **Paul [HH:MM GMT+7]:** "message text"
  
  **Audio archived: 2026-02-24-HHMMSS.ogg**
  
  **Jarvis [HH:MM GMT+7]:** Response text with proof/details
  ```

**Error #5: Designed infrastructure instead of executing existing spec**
- When asked "how can we make this reliable?", invented new systems instead of just following AUTO-LOGGING.md
- **Fix:** The spec already exists. Execute it exactly. Don't redesign, don't ask how, don't propose alternatives. Just do what the file says.

**Error #6: Didn't use state tracking**
- Didn't maintain which files had been processed, so I couldn't tell what was new vs old in inbound
- **Fix:** Create/update `.logstate.json` in `/memory/raw/YYYY-MM-DD/` tracking:
  ```json
  {
    "last_processed_timestamp": 1708750651,
    "processed_file_ids": ["uuid-1", "uuid-2"]
  }
  ```

---

## Corrected Session Startup Procedure (Feb 24+)

When a new session starts, execute these steps in order, showing proof:

1. **Create folder structure** — Run the mkdir command, verify folders exist
2. **Check for existing transcript.md** — Read first line. If exists, append mode. If not, create.
3. **Initialize .logstate.json** — Create with current timestamp if it doesn't exist
4. **Scan inbound and archive** — Copy any new files from `/Users/paulvisciano/.openclaw/media/inbound/` to archive with HHMMSS timestamps
5. **Log to transcript** — Append startup message with timestamp
6. **Then respond to Paul** — Say hello and ask what to do

Each step must show evidence before moving to the next.
