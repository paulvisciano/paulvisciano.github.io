...
Since day one the constraints were non-negotiable: it had to run on a normal 16GB laptop, and every part of the stack had to work with zero API keys.

No high-end GPU. No cloud credits. No rented inference. No keys sitting in environment variables that could leak or get revoked. The entire system — model, retrieval, voice, and spatial interface — had to live inside the machine I already owned.
...
The real test is simple: unplug the network. If it still works, nothing is leaving the machine. That offline test is the proof. There is no silent fallback to a remote endpoint, no hidden telemetry, no dependency that only reveals itself when the connection drops.
...
Five months in, the quietest win is still the most useful one: the models just stay put, they fit on hardware most people already have, and they pass the offline test every single day.
